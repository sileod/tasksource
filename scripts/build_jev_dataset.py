...
def source_licenses(sources):
    ...
    for source in sources:
        try:
            repos[source] = provenance_repos(source_provenance(source))
        except KeyError:
            repos[source] = []
    cards = fetch_card_licenses({repo for names in repos.values() for repo in names})
    return {source: source_license(source, names, cards) for source, names in repos.items()}
